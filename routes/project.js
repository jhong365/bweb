...
var ProjectPhoto = models.ProjectPhoto;
...
/*
 * request({ url : config.sbp.host + "comp/project/" + req.params.id, method :
 * 'GET' }, function(err, res, body) { if (!err && res.statusCode == 200) { var
 * json = JSON.parse(body); data['gig'] = json.project; data['userName'] =
 * json.profile? json.profile.name : json.project.account.email;
 * data['userAvatar'] = json.profile? json.profile.avatar : "/img/noimg.jpg"; }
 * else { console.log(err); data['gig'] = {}; } console.log(data);
